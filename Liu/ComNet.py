...
if __name__ == '__main__':
    ...
    for curr_fold in range(len(split_idxs)):
        ...
        model_chem3d = ComENet(cutoff=8.0,num_layers=4,hidden_channels=256,middle_channels=64,
            out_channels=1,num_radial=3,num_spherical=2,num_output_layers=3,)
        loss_func = torch.nn.MSELoss()
        evaluation = ThreeDEvaluator()
        run3d = run()
        run3d.run(curr_fold,modleName,device, train_dataset, valid_dataset, test_dataset, model_chem3d, loss_func, evaluation, epochs=1,
